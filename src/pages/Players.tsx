        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {players?.map((p: any) => (
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">{p.teams?.name || "Free Agent"}</p>
              <div className="mt-3 flex items-center gap-4 text-sm">
                {p.position !== "G" ? (
                  ...
                  <>
                    <span><strong className="text-foreground">{p.save_pct != null ? (Number(p.save_pct) * 100).toFixed(1) : "0.0"}%</strong> SV%</span>
                    <span><strong className="text-foreground">{p.shutouts ?? 0}</strong> SO</span>
                  </>
                )}
              </div>
            </Link>
          ))}
          {(!players || players.length === 0) && <p className="text-muted-foreground">No players yet.</p>}
        </div>
      )}
    </div>
  );
