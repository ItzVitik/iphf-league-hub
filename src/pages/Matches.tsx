...
const Matches = () => {
  ...
  const filtered = matches.filter((m) => {
    ...
    if (teamFilter !== "All" && m.teamA !== teamFilter && m.teamB !== teamFilter) return false;
    return true;
  });
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      <p className="text-muted-foreground mb-6">Season 1 — 2025</p>
        ...
        {filtered.map((match) => (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="font-heading font-bold text-lg text-foreground">{match.teamA}</span>
              {match.status === "Finished" ? (
                <span className="text-xl font-heading font-bold text-primary">{match.teamAScore} – {match.teamBScore}</span>
              ) : (
                <span className="text-sm text-muted-foreground">VS</span>
              )}
              <span className="font-heading font-bold text-lg text-foreground">{match.teamB}</span>
            </div>
            {match.mvp && (
              <div className="mt-2 text-xs text-gold">⭐ MVP: {match.mvp}</div>
            )}
          </Link>
        ))}
      </div>
